...
export function CourseArt({
  ...
  if (course.coverImage) {
    return (
      <div className={`relative overflow-hidden bg-black ${className}`}>
        ...
        <span className="absolute right-4 top-4 rounded-full border border-white/10 bg-black/40 px-3 py-1 font-sans text-[0.6rem] font-bold uppercase tracking-widest text-white/90 backdrop-blur-md">
          {course.category}
        </span>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <span className="absolute right-4 top-4 rounded-full border border-white/10 bg-white/5 px-3 py-1 font-sans text-[0.6rem] font-bold uppercase tracking-widest text-white/70 backdrop-blur">
        {course.category}
      </span>
    </div>
  );
