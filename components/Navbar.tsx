    ...
    <header className="fixed inset-x-0 top-0 z-50">
      <div
        className={`transition-all duration-500 ${
          scrolled
            ? "border-b border-white/[0.06] bg-ink/60 backdrop-blur-xl"
      ...
      >
        <div className="container-x flex h-16 items-center justify-between md:h-20">
          ...
          <button
            className="flex h-10 w-10 items-center justify-center rounded-full bg-white/[0.06] md:hidden"
            onClick={() => setOpen((v) => !v)}
            aria-label="เมนู"
            aria-expanded={open}
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="border-b border-white/[0.06] bg-ink/90 backdrop-blur-2xl md:hidden"
          >
            <nav className="container-x flex flex-col py-4">
              {nav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="border-b border-white/[0.06] py-3.5 text-lg text-white/80"
                >
                  {item.label}
                </Link>
              ))}
